 ...
 * (Z/p^rZ)[X]/(X^N-1) for some prime p and some exponent r, as defined in the NTRU Cryptosystems
 * Technical Report #14 "Almost Inverses and Fast NTRU Key Creation".
 *
 * <p>The prime p and the exponent r are supplied to the constructor, as a result each instance of
 * this class can compute inverses for the modulus p^r.
 *
 * <p>This algorithm will not work if the modulus is not a power of a prime number.
 */
public class PolynomialInverterModPowerOfPrime extends PolynomialInverterModPrime {
  /*
   * The exponent the prime p must be raised to to compute the modulus. That is, prime ^
   * powerOfPrime = modulus.
   */
  ...
  /*
   * Compute the inverse of a polynomial in (Z/p^rZ)[X]/(X^N-1) See NTRU Cryptosystems Tech Report
  ...
  public FullPolynomial invert(FullPolynomial a) {
    ...
    do {
      ...
      for (int i = 1; i < b.p.length; i++) c.p[i] = (short) (q - c.p[i]); // This is -c (mod q)
      //    iii: b = b*(2-a*b) mod q
