...
/*
 * This class provides utilities for packing values represented by 16-bit integers into byte arrays,
 * such that the packed elements use only the minimal number of bits in the output stream.
 *
 * <p>For example, given an array of values, each of which is in the range 0..2^9-1, the values
 * could be represented in java as a short (16 bit signed int). but would be packed into the output
 * array using 9 bits per element.
 */
public class BitPack {
  ...
  /*
   * Return an integer that can be used to mask off the low numBits of a value.
   *
   * @param numBits the number of bits to be masked off
   * @return a integer containing a bitmask where the least significant 'numbits' bits are set (1)
   *     and the remaining bits are clear (0).
   */
   ...
   * @param maxEltValue a maximal element value, larger than any input value (typically one larger
   *     than the max input value).
   * @return the number of elements required for the output array, or 0 on error.
   */
   ...
   * @param tgtOffset the offset into the destination array where the first element should go.
   * @param src the source array.
   * @param srcOffset the offset into the source array where the first value should be read from.
   * @return the number of elements used in the output array, or 0 on error.
   */
  public static int pack(
      int numElts, int maxEltValue, short[] src, int srcOffset, byte[] tgt, int tgtOffset) {
  ...
  /*
   * Bit-pack an array of shorts into a byte array, stopping after a predefined number of bytes have
   * been generated.
   *
   * @param numElts the number of elements
   * @param maxEltValue a maximal element value, larger than any input value (typically one larger
   *     than the max input value).
   * @param maxOutLen the maximum number of elements to put into the destination array.
   * @param tgt the destination array.
   * @param tgtOffset the offset into the destination array where the first element should go.
   * @param src the source array.
   * @param srcOffset the offset into the source array where the first value should be read from.
  ...
  /*
   * Calculate the number of input bytes that would be consumed decoding a bit-packed array.
   *
   * @param numElts the number of elements to decode
   * @param maxEltValue the maximum value that can appear in the array. This value is used to derive
   *     the number of bits per element in the packed representation.
  ...
  /*
   * Unpack a bit-packed array into an array of shorts. The number of bits per element is implied by
   * maxEltValue.
   *
   * @param numElts the number of elements to decode
   * @param maxEltValue the maximum value that can appear in the array. This value is used to derive
   ...
   * @param srcOffset the offset into the source array of the first element to be unpacked.
   * @param tgt the destination array
   * @param tgtOffset the offset into the destination array in which to start storing data.
   * @return the number of input bytes that were consumed, or 0 on error.
   */
  public static int unpack(
      int numElts, int maxEltValue, byte[] src, int srcOffset, short[] tgt, int tgtOffset) {
    // Get the number of bits in each element
    int bitsPerElement = countBits(maxEltValue - 1);
