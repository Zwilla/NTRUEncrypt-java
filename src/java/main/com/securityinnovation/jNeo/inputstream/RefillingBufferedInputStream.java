...
/*
 * This class buffers the output of an InputStream. It is parameterized by an InputStream that it
 * wraps and a buffer length.
 *
 * <p>This differs from the standard java.io.BufferedInputStream in that the standard BIS reads n
 * bytes of input from its wrapped input, and when that is consumed it consistently returns 0. This
 * class, on the other hand, reads n bytes of input from the wrapped input, doles it out, and when
 * it is all consumed it will request another n bytes of input from the wrapped InputStream.
 *
 * <p>This class was written to buffer the output of the X9.82 DRBG. The DRBG is defined to hash its
 * internal state to produce the output stream and discard any hash output in excess of the
 * requested length. This results in unnecessary hash calculations, especially when many small
 * requests are made. Wrapping the X9.82 in a RefillingBufferedInputStream and setting the buffer
...
public class RefillingBufferedInputStream extends InputStream {
   ...
   *     <p>See the parallel <code>read</code> method in <code>InputStream</code>.
   */
