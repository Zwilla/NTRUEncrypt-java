...
public class KeyFormatterUtilTestCase {
  ...
  @Test
  public void test_fillHeader_noOutput() {
    byte[] oid = {2, 9, 6};
  ...
  public void test_recoverF() throws NtruException {
    for (NtruEncryptTestVector test : tests) {
      ...
      FullPolynomial f = new FullPolynomial(test.f);
      ...
      for (int i = 0; i < f.p.length; i++) {
        if (f.p[i] > keyParams.q) f.p[i] %= keyParams.q;
        if (f.p[i] > keyParams.q / 2) f.p[i] -= keyParams.q;
      }
      FullPolynomial F = new FullPolynomial(test.F);
      FullPolynomial Fret = KeyFormatterUtil.recoverF(f);
      assertArrayEquals(F.p, Fret.p);
    }
  }
}
