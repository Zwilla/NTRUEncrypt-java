...
public class NtruEncryptKeyNativeEncoder implements NtruEncryptKeyEncoder {
  /*
   * The format for the public key. Format 1 consists of the following: 1 byte blob type tag ==
   * PUBLIC_KEY_v1 1 byte OID length <> bytes OID <> bytes h, bit packed.
   */
  public static final byte PUBLIC_KEY_v1 = 1;

  /*
   * The format for the private key blob where the private key is represented by the trinomial F,
   * packed 5 trits per output byte. Format 1 consists of the following: 1 byte blob type tag ==
   * PUBLIC_KEY_v1 1 byte OID length <> bytes OID <> bytes h, bit packed. <> bytes F, packed 5 trits
   * per output byte: out[i] = F[5*i]*3^4 + F[5*i+1]*3^3 + F[5*i+2]*3^2 + F[5*i+3]*3 + F[5*i+4]
   */
  public static final byte PRIVATE_KEY_DEFAULT_v1 = 2;

  /*
   * The format for the private key blob where the private key is represented by the trinomial F,
   * represented as a list of indices with coefficients of '1' (df of them) followed by a list of
   * indices with coefficients of '-1' (df of those). Each index is <N, so it bit-packed into
   * log2(N).
   */
  public static final byte PRIVATE_KEY_PACKED_F_v1 = (byte) 0xfe;

  public static final byte PRIVATE_KEY_LISTED_F_v1 = (byte) 0xff;
  ...
  PrivKeyFormatter pickDefaultPrivKeyFormatter(KeyParams keyParams) {
    ...
    int packedListedFLength = (keyParams.df * 2 * BitPack.countBits(keyParams.q + 7) / 8);
  ...
  public RawKeyData decodeKeyBlob(byte[] keyBlob)
    ...
    switch (keyBlob[0]) {
      ...
      case (PRIVATE_KEY_DEFAULT_v1):
        {
          PrivKeyFormatter formatter =
              pickDefaultPrivKeyFormatter(KeyFormatterUtil.parseOID(keyBlob, 1, 3));
          return formatter.decode(keyBlob);
        }
    }
