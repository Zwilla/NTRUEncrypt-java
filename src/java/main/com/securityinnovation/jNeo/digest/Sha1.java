...
class Sha1 extends Digest {
  ...
  /*
   * A temporary buffer used by the transform() routine It is allocated once per object to reduce
  ...
  /*
   * Convert an array of bytes into an array of 32-bit integers. Each output integer uses 4 input
   * bytes. The number of input bytes must be a multiple of 4.
   */
  ...
  /* Get the length of the hash output, in bytes. For SHA-1, this will always return 20. */
  ...
  /*
   * Get the size of the input block for the core hash algorithm, in bytes. For SHA-1, this will
   * always return 64.
   */
  ...
  public void finishDigest(byte[] out, int outOffset) {
    ...
    if (outOffset + getDigestLen() > out.length)
      throw new IllegalArgumentException(
          "output array is too short: start at offset "
              + outOffset
              + " and writing "
              + getDigestLen()
              + " bytes will overrun "
  ...
  /* Convert an integer array into an byte array using the big-endian encoing of the integers. */
