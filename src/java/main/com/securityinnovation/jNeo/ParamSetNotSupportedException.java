...
/*
 * This exception indicates that the input key blob contains a key that uses an NtruEncrypt or
 * NtruSign parameter set that is not supported by this implementation.
 */
public class ParamSetNotSupportedException extends NtruException {
  /*
   * Constructs a new exception with the supplied OID as the detail message, formatted as "w.x.y.z".
   */
  public ParamSetNotSupportedException(byte[] oid) {
    super("Ntru key parameter set (" + oidToString(oid) + ") is not supported");
  }

  /* Constructs a new exception with the supplied OID's name as the detail message. */
  ...
  private static String oidToString(byte[] oid) {
    ...
    if (oid.length > 0) s.append(oid[0]);
    for (int i = 1; i < oid.length; i++) s.append(".").append(0xff & oid[i]);
    return s.toString();
  }
}
