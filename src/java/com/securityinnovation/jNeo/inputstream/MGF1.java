...
/*
 * This InputStream derivative generates its output based on the MGF-1 algorithm defined in the
 * PKCS#1 spec.
 */
public class MGF1 extends InputStream {
   ...
   * @param _minNumRuns the minimum number of times the hash algorithm should be run. If the hash
   *     algorithm has not been run this many times before the MGF is close()ed, then during the
   *     close() the hash will be calculated enough times to bring the total count up to minNumRuns.
   * @param hashSeed a flag indicating whether the seed needs to be hashed before use. Typically
   *     this will be true if the seed length is greater than the block size of the underlying hash
   *     algorithm.
   * @param seed the array containing the MGF seed.
   * @param seedOffset the index of the start of the seed within the seed array.
  ...
  public MGF1(
    ...
    if (hashSeed) {
      seedAndCounter = new byte[Objects.requireNonNull(hash).getDigestLen() + 4];
    ...
    outputStream = new byte[Objects.requireNonNull(hash).getDigestLen()];
    outputUsed = outputStream.length;
  }

  /*
   * Closes this input stream. By the time this call returns the underlying hash function will have
   * been called minNumRuns times (see constructor)
   */
  public void close() {
    while (numRuns < minNumRuns) fillBuffer();
  ...
  /*
   * Update the internal state with the next block of output from the underlying hash algorithm..
   */
