...
/*
 * This class implements an InputStream whose output conforms to the X9.82 specification for "Hash
 * Function DRBG Using Any Approved Hash Function" (section 10.1.3 of ANS X9.82, Part 3, Draft July
 * 2003).
 *
 * <p>In addition to the InputStream API, this class also adds functions for seeding and reseeding
 * the DRBG.
 */
public class X982Drbg extends InputStream {
  ...
  /* The seed vector, called V in the X9.82 spec. Updated in each call to generate random bytes. */
  private byte[] V;
  /*
   * This is a copy of V that is incremented during the internal hashGen method. It is preallocated
   * once per DRBG to avoid the overhead of allocation.
   */
  private byte[] Vtmp;
  ...
  /*
   * Implement the Initialize_Hash_DRBG algorithm from the X9.82 spec. This implementation does not
   * support an application purpose (the variable t). It also does not enforce the minimum entropy
  ...
  /*
   * Implement the Reseed_Hash_DRBG algorithm from the X9.82 spec. This implementation does not
   * support an application purpose (the variable t). It also does not enforce the minimum entropy
   * requirements.
   */
  ...
  /*
   * Implement the Hash_DRBG algorithm from the X9.82 spec. This implementation does not support any
   * user input.
   */
  public int read() {
    byte[] b = new byte[1];
    ...
    if (offset + len > out.length)
      throw new IllegalArgumentException(
          "Writing "
              ...
              + " bytes long)");
    if (offset < 0)
      throw new IllegalArgumentException("Output array offset is negative (" + offset + ")");
  ...
  /*
   * Implement the Hashgen subroutine of the X9.82 Hash_DRBG algorithm: calculate H(V) | H(V+1) |
   * H(V+2) | ... until len bytes are generated. Store the result in out[offset...].
   */
  ...
  /* Add two multi-byte big-endian integers: accum += in[offset..len-1]. Overflow is discarded. */
  ...
  /* Add x to a multi-byte big-endian integer: accum += x. Overflow is discarded. */
