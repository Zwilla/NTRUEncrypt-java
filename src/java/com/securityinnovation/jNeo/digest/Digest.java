...
/*
 * This class defines an interface for digest operations. It is not declared as an interface because
 * it provides a number of utility methods for common operations on digest objects.
 *
 * <p>There are number of coventions used by all implementations of all methods in the digest
 * subclasses:
 *
 * <UL>
 *   <LI>All buffers must be non-null; a <code>NullPointerException</code> will be thrown if any
 *       buffer is null. This is true even if the input length is 0.
 *   <LI>In all cases where an offset and a length are supplied to indicate the input or output
 *       range, an <code>IllegalArgumentException</code> will be thrown if the specified range of
 *       the buffer overruns the bounds of the array.
 *   <LI>In all cases where an offset is supplied for an input or output buffer, an <code>
 *       IllegalArgumentException</code> will be thrown if the offset is negative.
 *   <LI>In all cases where a length is supplied for an input or output buffer, an <code>
 *       IllegalArgumentException</code> will be thrown if the length is negative.
 * </UL>
 */
public abstract class Digest {
  ...
  /*
   * Calculate the digest of the input and return the result. This discards any existing internal
   * state of the object. The object is left initialized for a new hash operation (see <code>reset()
   * </code>).
   *
   ...
   * @param outputOffset the index where the first byte of output should go. This will use
   *     getDigestLen() bytes following this index.
   */
  public void digest(
  ...
  /*
   * Completes the digest calculation and returns the result in the supplied array. The output will
   * be <code>getDigestLen()</code> bytes long. The object is reinialized (see <code>reset()</code>
   * ).
   */
  public abstract void finishDigest(byte[] out, int outOffset);


  /* Completes the digest calculation and returns the result in a newly allocated array. */
