...
public class NtruEncryptKeyBBTestCase {
  ...
  public void test_getPubKey_known_value() throws NtruException {
    for (OID oid : OID.values()) {
      ...
      if (oid == OID.ees401ep1) assertEquals(blob.length, 556);
      else if (oid == OID.ees449ep1) assertEquals(blob.length, 622);
      else if (oid == OID.ees677ep1) assertEquals(blob.length, 935);
      else if (oid == OID.ees1087ep2) assertEquals(blob.length, 1499);
      else if (oid == OID.ees541ep1) assertEquals(blob.length, 748);
      else if (oid == OID.ees613ep1) assertEquals(blob.length, 847);
      else if (oid == OID.ees887ep1) assertEquals(blob.length, 1224);
      else if (oid == OID.ees1171ep1) assertEquals(blob.length, 1615);
      else if (oid == OID.ees659ep1) assertEquals(blob.length, 911);
      else if (oid == OID.ees761ep1) assertEquals(blob.length, 1051);
      else if (oid == OID.ees1087ep1) assertEquals(blob.length, 1499);
      else if (oid == OID.ees1499ep1) assertEquals(blob.length, 2066);
    }
  }
  ...
  public void test_positive() throws NtruException {
    for (OID oid : OID.values()) assertTrue(runFullTest(oid));
  }

  // Find a test vector for the specified parameter set.
  NtruEncryptTestVector findTest(OID oid) {
    for (NtruEncryptTestVector test : tests) if (oid == test.oid) return test;
    return null;
  }
  ...
  byte[] getPrivKeyBlob(NtruEncryptTestVector tv) {
    byte[] encodedF = null;
    if (tv.packedF.length < tv.packedListedF.length) encodedF = tv.packedF;
    else encodedF = tv.packedListedF;
