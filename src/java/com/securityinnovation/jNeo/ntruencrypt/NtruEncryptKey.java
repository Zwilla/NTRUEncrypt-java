...
/*
 * This class represents NtruEncrypt keys. An instance may hold a public/private keypair or just the
 * public key.
 */
public class NtruEncryptKey {
  /* The parameter set for this key. */
  protected KeyParams keyParams;

  /* The public key. */
  protected FullPolynomial h = null;

  /* The private key f (= 1+p*F). For public-key-only objects this will be null. */
  ...
  /*
   * Create a new key from the supplied keyblob. The key blob can be either a public or private key
   * blob generated with <code>getPubKey()</code> or <code>getPrivKey()</code>.
   */
  public NtruEncryptKey(byte[] keyBlob)
  ...
  /*
   * Create an empty key object associated with the supplied parameter set. This is for use in unit
   * testing where the key material is not used.
   */
  ...
  /*
   * Generate a new keypair for the specified parameter set. (see com.securityinnovation.jNeo.OID
   * for a list of parameter set identifiers).
   */
  ...
  /*
   * Generate a new keypair for the specified parameter set using the supplied InputStream as a
  ...
  static NtruEncryptKey genKey(OID oid, InputStream prng) throws ParamSetNotSupportedException {
    ...
    FullPolynomial F, f = null, fInv = null;
  ...
  /*
   * Signal that this app is finished using this key. This clears all sensitive memory stored by the
   * object and makes the object unusable.
   */
  public void close() {
    if (h != null) java.util.Arrays.fill(h.p, (byte) 0);
    h = null;
    if (f != null) java.util.Arrays.fill(f.p, (byte) 0);
    f = null;
  }
  ...
  public byte[] decrypt(byte[] ciphertext)
      throws NoPrivateKeyException, CiphertextBadLengthException, ObjectClosedException,
  ...
  short[] convPolyBinaryToTrinary(int outputDegree, byte[] bin) {
    ...
    for (int i = 0; i < blocks; i++) {
      int val =
          (((0xff & bin[i * 3]) << 16) | ((0xff & bin[i * 3 + 1]) << 8) | (0xff & bin[i * 3 + 2]));
