...
/*
 * This class implements the algorithm for finding the inverse of a polynomial in the ring
 * (Z/pZ)[X]/(X^N-1) for some prime p, as defined in the NTRU Cryptosystems Technical Report #14
 * "Almost Inverses and Fast NTRU Key Creation".
 *
 * <p>The prime modulus is supplied to the constructor, as a result each instance of this class can
 * compute inverses for one modulus.
 *
...
public class PolynomialInverterModPrime implements PolynomialInverter {
  ...
  /*
   * A table of inverses mod <code>prime</code>. The table should be set up so that invModPrime[i] *
   * i = 1 (mod prime) if the inverse of i exists invModPrime[i] = 0 if the inverse of i does not
   * exist.
   */
  protected short[] invModPrime;

  /*
   * This constructor initializes the object to calculate inverses modulo a particular prime.
   *
   * @param _prime the modulus.
   * @param _invModPrime a precomputed table of integer inverses modulo _prime. The table should be
   *     initialized so that invModPrime[i] * i = 1 (mod prime) if the inverse of i exists,
   *     invModPrime[i] = 0 if the inverse of i does not exist. This table should not be modified
  ...
  protected static int getDegree(final FullPolynomial f) {
    int df = f.p.length - 1;
    while ((df > 0) && f.p[df] == 0) df--;
    return df;
  }


  /*
   * Divide f(X) by the polynomial g(X)=X. f(X) is updated in-place. This effectively a
   * "shift-left-with-wraparound" of the indices.
   */
  protected static void divideByX(FullPolynomial f) {
    short f0 = f.p[0];
    System.arraycopy(f.p, 1, f.p, 0, f.p.length - 1);
    f.p[f.p.length - 1] = f0;
  }


  /*
   * Mulitply f(X) by the polynomial g(X)=X. f(X) is updated in-place. This effectively a
   * "shift-right-with-wraparound" of the indices.
   */
  protected static void multiplyByX(FullPolynomial f) {
    short fn = f.p[f.p.length - 1];
    System.arraycopy(f.p, 0, f.p, 1, f.p.length - 1);
    f.p[0] = fn;
  }


  /*
   * Compute the inverse of a polynomial in (Z/pZ)[X]/(X^N-1) See NTRU Cryptosystems Tech Report
  ...
  /*
   * Returns x mod prime, always in the range [0..prime-1]. This differs from the % operator by
   * returning a positive result when fed a negative number.
   */
