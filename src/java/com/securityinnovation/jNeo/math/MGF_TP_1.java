...
/*
 * This class implements the MGF-TP-1 algorithm for converting a byte stream into a sequence of
 * trits. It implements both the forward direction (bytes -> trits as defined in X9.98) and the
 * reverse (trits -> bytes).
 */
public class MGF_TP_1 {
  /*
   * Generate a trinomial of degree N using the MGF-TP-1 algorithm to convert the InputStream of
   * bytes into trits.
   *
  ...
  /*
   * Given a trit in the range [0..2], return a trit in the range [-1..1] that is equal to the input
   * mod 3.
   */
  ...
  /*
   * Generate a byte stream that is the encoding of a trinomial. The byte stream will have the
   * property that when it is run through the MGF-TP-1 algorithm it will recover the original
   * trinomial.
   *
  ...
  public static void encodeTrinomial(FullPolynomial poly, OutputStream out) {
    try {
      ...
      if (end < N) {
        ...
        while (end < N) accum = 3 * accum + recenterTritTo0(poly.p[--N]);
        out.write(accum);
      }
    } catch (IOException e) {
