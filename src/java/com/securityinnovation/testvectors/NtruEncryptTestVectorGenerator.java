...
public class NtruEncryptTestVectorGenerator {
  ...
  private static byte[] makeSeed(OID oid, String usage) throws IOException {
  ...
  public static void main(String[] args) throws NtruException, IOException {
    for (OID oid : OID.values()) {
      // TVDump.setOID(oid);
      byte[] oidBytes = OIDMap.getOIDBytes(oid);
      System.out.println(
          "    // OID = " + oid + "  " + oidBytes[0] + "." + oidBytes[1] + "." + oidBytes[2]);
